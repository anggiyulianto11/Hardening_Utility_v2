...
from PySide6.QtWidgets import QApplication,QCheckBox,QFileDialog,QFormLayout,QGroupBox,QHBoxLayout,QLabel,QLineEdit,QMainWindow,QMessageBox,QPushButton,QRadioButton,QTableWidget,QTableWidgetItem,QVBoxLayout,QWidget
...
class ConnectionWorker(QObject):
    ...
    def __init__(self,mode,url,username,password,verify_tls): super().__init__(); self.mode,self.url,self.username,self.password,self.verify_tls=mode,url,username,password,verify_tls
    def run(self):
        try:
            registry=ConnectionRegistry(verify_tls=self.verify_tls); result=registry.connect_enterprise(self.url,self.username,self.password) if self.mode=="enterprise" else registry.connect_standalone(self.url,self.username,self.password); self.password=""; self.completed.emit(result)
        except Exception as exc: self.password=""; self.failed.emit(redact_text(exc))
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__(); self.setWindowTitle("ArcGIS Enterprise Hardening Utility v2 - Milestone 1 Patch 1"); self.resize(1500,760); self.thread=None; self.worker=None; self.current_result=None
        root=QWidget(); layout=QVBoxLayout(root); self.setCentralWidget(root); title=QLabel("Connection and Target Discovery"); title.setStyleSheet("font-size: 22px; font-weight: 700;"); layout.addWidget(title)
    ...
    def update_mode(self):
        enterprise=self.enterprise_radio.isChecked(); self.form.labelForField(self.url_input).setText("Portal URL:" if enterprise else "Server URL:"); self.url_input.setPlaceholderText("https://host/portal" if enterprise else "https://host/server"); self.connect_button.setText("Connect & Discover" if enterprise else "Connect Standalone Server"); self.table.setRowCount(0); self.current_result=None; self.export_button.setEnabled(False); self.summary.setText("Belum terhubung.")
    def connect(self):
    ...
    def on_completed(self,result):
        self.connect_button.setEnabled(True); self.password_input.clear(); self.current_result=result; self.export_button.setEnabled(True); self.table.setRowCount(len(result.targets))
        ...
        self.table.resizeColumnsToContents(); self.summary.setText(f"Discovery selesai. {result.connected_count} dari {len(result.targets)} target dapat diakses.")
...
def run_app(): app=QApplication(sys.argv); window=MainWindow(); window.show(); sys.exit(app.exec())
